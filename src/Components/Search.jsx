...
const SearchBar = ({ type }) => {
    ...
    const handleSearch = (event) => {
        if (event.key === 'Enter' && query) {
            ...
            navigate(`/search?type=${type}&q=${formattedQuery}`);
        }
    };
    ...
    return (
        <div className="md:w-[700px] h-[3rem] flex relative items-center border rounded-full m-auto">
            <input
                type="text"
                value={query}
            ...
            <button className='w-[20%] md:w-[10%] h-full flex justify-center items-center px-2 rounded-r-full bg-[#222222]'
                onClick={() => {
                    if (query !== "") {
                        const formattedQuery = query.replace(/\s+/g, '-');
                        navigate(`/search?type=${type}&query=${formattedQuery}`);
                    }
                }}>
                <SearchIcon />
            </button>
